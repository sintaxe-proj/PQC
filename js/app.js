...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Botões de Acesso Rápido do SVG
    document.getElementById('btn-reset-mapa')?.addEventListener('click', () => bodyMap.limparSelecao?.() || location.reload());
    document.getElementById('btn-select-frente')?.addEventListener('click', () => bodyMap.selecionarGrupo?.('visao-anterior'));
    document.getElementById('btn-select-verso')?.addEventListener('click', () => bodyMap.selecionarGrupo?.('visao-posterior'));

    // Preenche a data/hora do acidente por padrão
    ...
    // Identificação de Idade e Ajuste Lund-Browder
    ...
    if (btnCalcular) {
        btnCalcular.addEventListener('click', () => {
            ...
            document.getElementById('res-vol-total').innerText = `${volumeTotal24h.toLocaleString('pt-BR')} mL`;
            document.getElementById('res-vazao-ataque').innerText = `${vazaoAtaqueMLh.toFixed(1)} mL/h`;
            
            // Diurese e Ajuste de Bomba
            ...
            document.getElementById('res-perda-evaporativa').innerText = `${balancoResult.perdaEvaporativa24h.toLocaleString('pt-BR')} mL`;
            const elBalanco = document.getElementById('res-balanco');
            if (elBalanco) {
                const sinal = balancoResult.balancoFinal > 0 ? '+' : '';
                elBalanco.innerText = `${sinal}${balancoResult.balancoFinal.toLocaleString('pt-BR')} mL`;
            }
            ...
            // Módulo Pediátrico (Holliday-Segar)
            ...
            if (idadeCalculada < 15) {
                ...
                if (holliday && painelPeds && resHolliday) {
                    ...
                    resHolliday.innerHTML = `<strong>Manutenção Basal:</strong> ${holliday.volume24h} mL/24h (${holliday.vazaoHora} mL/h)<br><strong>Composição:</strong> ${holliday.solucaoSugerida}`;
                }
            } else if (painelPeds) {
                painelPeds.style.display = 'none';
            }

            // Alerta de Fluid Creep / Síndrome Compartimental
    ...
    // Exportar Evolução para o Prontuário Eletrônico
    if (btnCopiar) {
